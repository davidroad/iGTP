...
class GEARS:
    ...
    def plot_perturbation(self, query, save_file=None):
        """
        Plot the perturbation graph using violin plots for top 10 genes.
        ...
        de_idx = [gene2idx[gene_raw2id[i]] for i in
                  adata.uns['top_non_dropout_de_20'][cond2name[query]][:10]]
        ...
        # Replace boxplot with violinplot for top 10 genes
        sns.violinplot(data=truth, inner=None)
    
        # Add scatter plot for predicted values
        for i in range(pred.shape[0]):
            _ = plt.scatter([i] * len(pred[i]), pred[i], color='red')
